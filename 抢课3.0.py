...
def grab_lessons(id,lesson_class):
    ...
    try:
        ...
        if json_message['success']==False:
            print("未抢到该课程",json_message['message'])
            time.sleep(delay)
            grab_lessons(id,lesson_class)
        else:
            print('抢课成功',json_message['message'])
            input("回车退出")
    except:
        print("cookies失效 或 服务器请求超时重新运行程序 或 选课成功后服务器无响应(过段时间登录查看)")
    ...
    while True:
        if lesson_num.isdigit()==True:
            for i in range(int(lesson_num)):
                lesson_id=input("想要选择的课程(课程号+课序号,示例:21210300191301):").strip()
